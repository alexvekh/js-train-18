...
function validateUser(user) {
  ...
  try {
    if (typeof user !== "object" && user == null) throw new Error("Об'єкт користувача не вказано!");
    if (!user.name) throw new Error("Ім'я користувача не вказано!", { cause: user });
    if (!user.email) throw new Error("Email користувача не вказано!", { cause: user });
...
function calculateSquareRoot(number) {
  ...
  try {
    if (typeof number !== "number") throw new TypeError("Аргумент має бути числом!");
...
function processData(data) {
  ...
  try {
    for (const [index, element] of data.entries()) {
      if (typeof element !== "number") throw new TypeError(`Елемент з індексом ${index} має бути числом!`);
    }
